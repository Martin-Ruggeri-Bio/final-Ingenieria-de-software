...
class AddLike(LoginRequiredMixin, View):
    def post(self, request, pk, *args, **kwargs):
        post = SocialPost.objects.get(pk=pk)
        is_dislike = False
        # por cada dislike en el post, si el usuario dio dislike se encuentra, se da dislike
        for dislike in post.dislikes.all():
            if dislike == request.user:
                is_dislike = True
                break
        # si tiene un dislike hay que removerlo
        if is_dislike:
            post.dislikes.remove(request.user)
        # por cada like en el post, si el usuario dio like se encuentra, se da like
        is_like = False
        for like in post.likes.all():
            if like == request.user:
                is_like = True
                break
        # si no tiene like hay que agregarle un like
        if not is_like:
            post.likes.add(request.user)
        # si ya tiene like hay que removerlo
        if is_like:
            post.likes.remove(request.user)
        # redirijimos nuestro hhtpresonse a la proxima url
